...
To double resource drops, set `double_resources = true`. For an existing server, append `-modifier resources muchmore` to `SERVER_ARGS` in `/etc/valheim.env`, preserving other modifiers, then restart the `valheim` service normally. This applies to the current world without changing its name, seed, terrain, or player progress. Save and back up the current world before maintenance. Like other world modifiers, it persists in the save; setting the Terraform option back to false only removes the startup override and does not reset an existing world's multiplier.

For gentler deaths, set `casual_death_penalty = true` (keep equipped gear and greatly reduce skill loss). Set `less_raids = true` to reduce raid frequency. Existing servers need `-modifier deathpenalty casual -modifier raids less` appended to `SERVER_ARGS` and a normal service restart after saving and backing up. These options leave combat and player-based raid rules unchanged. As with the other modifiers, setting the Terraform booleans back to false does not reset values already stored in a world save.
